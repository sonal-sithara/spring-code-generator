...
async function createApiDocumentation() {
    try {
        ...
        vscode.window.showErrorMessage(`Error creating API documentation: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
}
...
async function generateApiDocumentationFiles(workspaceFolder, config) {
    ...
    await vscode.workspace.fs.writeFile(applicationYmlUri, Buffer.from(applicationYmlContent));
}
/**
 ...
 */
function generatePomDependencies() {
 ...
 * Generate application.yml configuration
 */
function generateApplicationYmlConfig() {
 ...
 */
function generateEndpointDocumentation(controllerName, methodName, httpMethod, endpoint) {
    return `
...
exports.generateEndpointDocumentation = generateEndpointDocumentation;
