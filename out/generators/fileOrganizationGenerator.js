...
exports.analyzeProjectStructure = exports.organizeProjectFiles = void 0;
 ...
 */
async function organizeProjectFiles() {
    try {
        ...
        const javaFiles = files.filter(([name, type]) => type === vscode.FileType.File && name.endsWith(".java"));
        ...
        vscode.window.showErrorMessage(`Error organizing files: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
}
exports.organizeProjectFiles = organizeProjectFiles;
/**
 * Organize a single file based on naming conventions
 */
async function organizeFile(workspaceFolder, srcPath, fileName) {
    try {
 ...
 */
function determineFolderFromFileName(fileName) {
    ...
    // Service detection
    if ((lowerName.includes("service") && lowerName.endsWith(".java")) ||
    ...
    // Entity detection
    if ((lowerName.endsWith("entity.java") ||
 ...
 * Analyze file distribution
 */
function analyzeFileDistribution(fileNames) {
...
function createAnalysisReport(analysis) {
    const totalFiles = Object.values(analysis.folderDistribution).reduce((a, b) => a + b, 0) +
...
//# sourceMappingURL=fileOrganizationGenerator.js.map